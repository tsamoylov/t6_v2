...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    else if (*v)
    {
        ...
        window->DrawList->AddRectFilled(check_bb.Min + pad, check_bb.Max - pad, check_col, 3.5f);
...
bool ImGui::ColorEdit3(const char* label, float col[3], ImGuiColorEditFlags flags, bool* rgb)
{
    return ColorEdit4(label, col, flags | ImGuiColorEditFlags_NoAlpha, rgb);
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags, bool* rgb)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
